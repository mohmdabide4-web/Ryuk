...
// ==== رقم المالك ====
const OWNER = "212726590815@s.whatsapp.net";

// ==== رقم البوت (للـ Pairing Code) ====
const BOT_NUMBER = "212781399940";
...
// تنظيف النص من الهمزات والتشكيل والمسافات الزائدة
...
function findAnime(text) {
  ...
  if (!isNaN(asNumber)) {
    return config.animeList.find((a) => a.id === asNumber);
  }

  return config.animeList.find((a) => {
...
async function startBot() {
  ...
  // Pairing Code تلقائي (بدون إدخال يدوي)
    ...
    if (sender === OWNER && !from.endsWith("@g.us")) {
      if (text === "الألقاب" || text === "الالقاب") {
        const list = Object.entries(claims)
          .map(([id, number]) => {
            const anime = config.animeList.find(a => a.id == id);
...
async function handleMessage(sock, from, text, messageKey) {
  ...
  if (!state.greeted) {
    await sendNaturally(sock, from, { text: config.welcomeMessage }, messageKey);
  ...
  if (anime) {
    ...
    if (claimedBy && claimedBy !== from) {
      await sendNaturally(sock, from, { text: config.takenMessage }, messageKey);
      return;
    }
    ...
    await sendNaturally(
      sock,
      from,
      { text: config.groupLinkMessage(anime.name, config.mainGroupLink) },
      messageKey
    );
    return;
  }
  ...
  if (isReal) {
    await sendNaturally(sock, from, { text: config.knownButUnsupportedMessage }, messageKey);
  } else {
    await sendNaturally(sock, from, { text: config.invalidChoiceMessage }, messageKey);
  }
}
